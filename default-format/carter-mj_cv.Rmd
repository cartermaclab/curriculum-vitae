...
Decision-making for action; Human-robot and Human-human interactions; Neuroeconomics of action control; Feedback; Aging; Metascience
...
2014, Franklin Henry Young Scientist Award (Motor Control and Learning), Canadian Society for Psychomotor Learning & Sport Psychology, $300.
...
tibble(
  Years = "2019-20",
  Role = "Reviewer",
  Name = "Natural Sciences and Engineering Research Council of Canada 2020 Discovery
Grant Competition, Biological Systems and Functions",
  #English = cell_spec(c("C2","C1","C2","C2"), 'latex', color = '#009acd'), # #009acd
  #French = cell_spec(c("B2","B2","B2","B2"), 'latex', color = '#009acd'), # #4cb8dc
) %>%
  kbl(format = "latex", booktabs = TRUE, align = 'l', escape = F, longtable = TRUE) %>%
...
```{r}
tibble(
  Years = c("2020-", rep("2019-", 2), "2018", rep("2018-20", 2), rep("2017-19", 4)),
  ...
  Status = c(rep("In Progress", 3), rep("Completed", 7)),
  #English = cell_spec(c("C2","C1","C2","C2"), 'latex', color = '#009acd'), # #009acd
  #French = cell_spec(c("B2","B2","B2","B2"), 'latex', color = '#009acd'), # #4cb8dc
) %>%
  kbl(format = "latex", booktabs = TRUE, align = 'l', escape = F, longtable = TRUE) %>%
...
```{r}
tibble(
  Year = c(rep("2021", 5), rep("2020", 3), rep("2019", 4), rep("2018", 4)),
  Name = c("Anita Chaseendran", "Gianna Jeyarajan", "Kenzie Bell", "Ashton Yuen", "Charlie Violin", "Ammaar Jan", "Faran Chaudhry", "Yu (Sarah) Fu", " Andrew Poskus", "Maya El-Zahed", "Grant Yang", "Olena Leshchyshen", "Rita Waseem", "Christian Kleiser", "Chris Li", "Faryal Zahir"),
  Program = c(rep("Life Sciences", 2), rep("Kinesiology", 4), rep("Health Sciences", 2), rep("Kinesiology", 4), "Life Sciences", rep("Kinesiology", 2), "Biology"),
  Level = c(rep("III", 5), rep("IV", 2), rep("III", 8), "IV"),
  Institution = c(rep("McMaster University", 16)),
  #English = cell_spec(c("C2","C1","C2","C2"), 'latex', color = '#009acd'), # #009acd
  #French = cell_spec(c("B2","B2","B2","B2"), 'latex', color = '#009acd'), # #4cb8dc
) %>%
  kbl(format = "latex", booktabs = TRUE, align = 'l', escape = F, longtable = TRUE) %>%
...
[2]. ^+^St. Germain L, ^+^Williams A, ^+^Balbaa N, ^+^Poskus A, ^+^Leshchyshen O, Lohse KR, **Carter MJ**. Increased perceptions of autonomy through choice fail to enhance motor skill retention. *PsyArXiv*. doi: [10.1101/2020.09.02.279414](https://doi.org/10.1101/2020.09.02.279414)
...
[23.] ^+^McKay B, Hussien J, **Carter MJ**, Yantha Z, & Ste-Marie DM. (2021). Expecting to teach a novel golf putting task did not enhance retention performance: A replication attempt of Daou and colleagues. *Communications in Kinesiology*, 1: 1-21.

[22.] McKay B, Yantha ZD, Hussien J, **Carter MJ**, & Ste-Marie DM. (In-Press). Meta-analytic findings in the self-controlled motor learning literature: Underpowered, biased, and lacking evidential value. *Meta-Psychology*.
